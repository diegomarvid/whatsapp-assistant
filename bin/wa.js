...
import { REVIEW_TIME_ZONE, formatReviewLocalTimestamp, reviewWindow, selectConversationReview } from '../src/conversation-review.js'
...
Comandos:
  ...
  wa review <alias or phone> [--date today|yesterday|YYYY-MM-DD [--start HH:MM] [--end HH:MM] | --since 12h|7d [--until now|YYYY-MM-DD]] [--from incoming|me|any] [--any term...|--all term...] [--context 4] [--ids|--json]
...
function parseReviewArguments(args) {
  const options = { date: null, start: null, end: null, since: null, until: null, mode: 'any', from: 'any', terms: [], context: 4, ids: false, json: false }
  while (args.length) {
    const option = args.shift()
    if (option === '--date' || option === '--start' || option === '--end' || option === '--since' || option === '--until' || option === '--context') {
      const value = args.shift()
      if (!value) throw new Error(`Use ${option} <value>`)
      if (option === '--date') options.date = value
      else if (option === '--start') options.start = value
      else if (option === '--end') options.end = value
...
async function agentsCommand(args) {
  ...
  if (action === 'profile') {
    ...
    if (profileAction === 'list') {
      ...
      for (const profile of profiles) console.log(`${profile.name} — ${profile.provider}/${profile.model}${profile.effort ? ` (effort ${profile.effort})` : ''}${profile.reasoningEffort ? ` (reasoning ${profile.reasoningEffort})` : ''}${profile.workspace ? `; workspace ${profile.workspace.path}` : ''}`)
      return
    }
    ...
    if (profileAction === 'set') {
      ...
      const workspace = extractOption(args, '--workspace')
      ...
      const profile = await agentProfiles.set({ name, ...(provider ? { provider } : {}), ...(model ? { model } : {}), ...(clearEffort ? { effort: null, reasoningEffort: null } : effort ? { effort } : reasoningEffort ? { reasoningEffort } : {}), ...(promptFile ? { promptFile } : {}), ...(workspace ? { workspace } : {}), ...(timeoutMs ? { timeoutMs } : {}) })
...
async function automationCommand(args) {
  ...
  if (action === 'list') {
    ...
    const profiles = verbose ? new Map((await agentProfiles.list()).map((profile) => [profile.name, profile])) : null
    for (const [index, rule] of rules.entries()) {
      const report = formatPromptAutomation(rule, batches[index])
      if (!verbose) {
        console.log(report)
        continue
      }
      const profile = profiles.get(rule.profile)
      if (!profile) {
        console.log(`${report}\n  IA: perfil ${rule.profile} ya no existe`)
        continue
      }
      const prompt = await promptHealth(profile)
      console.log(`${report}\n  IA: ${profile.provider}/${profile.model}${profile.effort ? `; effort ${profile.effort}` : ''}${profile.reasoningEffort ? `; reasoning ${profile.reasoningEffort}` : ''}\n  Prompt: ${profile.prompt.path} (sha256 ${profile.prompt.sha256.slice(0, 12)}…; ${prompt.status})\n  Workspace: ${profile.workspace?.path || 'none (WhatsApp-only agent)'}\n  Timeout: ${profile.timeoutMs} ms`)
    }
    return
  }
  ...
  if (command === 'review') {
    ...
    const report = {
      ...
      window: {
        ...
        localSince: formatReviewLocalTimestamp(window.since, window.timeZone),
        localUntil: formatReviewLocalTimestamp(window.until, window.timeZone),
      },
      query: {
        mode: options.mode,
        from: options.from,
      ...
      matches: selected.matching.map((entry) => ({
        ...
        localTimestamp: formatReviewLocalTimestamp(entry.message.timestamp, window.timeZone),
        matchedTerms: entry.matchedTerms,
      })),
      timeline: selected.timeline.map((message) => ({
        ...message,
        localTimestamp: formatReviewLocalTimestamp(message.timestamp, window.timeZone),
      })),
    ...
    console.log(`Cobertura: ${coverage.status} · ventana ${report.window.localSince} → ${report.window.localUntil} · ${window.timeZone}`)
