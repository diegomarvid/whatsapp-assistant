...
async function processPromptAutomations() {
  if (!automationWorker && promptAutomations && agentProfiles && automationCapabilities) {
    automationWorker = new AutomationWorker({ rules: promptAutomations, profiles: agentProfiles,
      ...
      coverage: chatCoverage, resolveJid: resolveCurrentJid, messages: () => cache.messages, logger })
  }
...
function automationEndpointAllowed(request, url, { isAudioDownload, isImageDownload, isDocumentDownload, isVideoDownload, isStickerDownload, isMessageSend }) {
  return (request.method === 'GET' && ['/resolve', '/coverage', '/messages', '/identities'].includes(url.pathname))
    ...
    || (request.method === 'POST' && ['/automation/decision', '/automation/result'].includes(url.pathname))
...
// WhatsApp suppresses phone push notifications while a companion looks like
// the client the user is reading — that is decided by PRESENCE, not by the
// active/passive delivery mode. Marking presence unavailable keeps phone
// notifications flowing while the connection stays active, so messages are
// pushed in real time. (A passive delivery mode also preserved notifications
// but made the server batch message delivery every ~5 minutes.)
async function markPresenceUnavailable() {
  try {
    await socket.sendPresenceUpdate('unavailable')
    cache.sync.presenceUnavailableAssertedAt = nowSeconds()
    cache.sync.passiveModeAssertedAt = null
    logger.info('Marked presence unavailable so phone notifications keep arriving')
  } catch (error) {
    cache.sync.presenceUnavailableAssertedAt = null
    logger.warn({ err: error }, 'Could not mark presence unavailable')
  }
}
...
async function main() {
  ...
  const server = http.createServer((request, response) => {
    ...
    const isAutomationWrite = request.method === 'POST' && ['/automation/decision', '/automation/result', '/automation/preview'].includes(url.pathname)
